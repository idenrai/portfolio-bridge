...
/**
 * 구루별 팔로우업 프롬프트 전용 평가 포커스
 * (첫 번째 full-review용 lens/format과 별도로, 변화 점검에 특화된 기준)
 */
export const GURU_FOLLOWUP_FOCUS: Record<string, string> = {
  buffett: `For each new position: does it have an identifiable economic moat (brand, switching costs, network effects, cost advantage) you can articulate in one sentence? For each removal: was the exit because the business fundamentally deteriorated, or because the price moved against you temporarily? For weight changes: are the largest positions now your highest-conviction, most understandable businesses — or has comfortable drift elevated mediocre names? Close with a verdict on whether the portfolio is moving toward or away from the kind of concentrated, owner-operated quality portfolio you would build for your grandchildren.`,

  munger: `Invert first: did the changes eliminate the most obvious causes of long-term failure — complexity, poor incentive alignment, deteriorating moats — or did they introduce new ones? For each new addition, apply the stupidity filter: would a rational person with a 20-year horizon pay today's price for this business, or is it fashionable noise? Are removed positions corrections of psychological misjudgments (loss aversion, commitment bias, sunk-cost thinking), or was rational long-term thinking abandoned for short-term comfort? State the one remaining cognitive mistake most visibly embedded in the current portfolio.`,

  lynch: `Re-run the Lynch category classification for each new addition (Slow Grower / Stalwart / Fast Grower / Cyclical / Turnaround / Asset Play) and confirm the story still holds in two plain sentences. Recalculate PEG ratios for all growth additions — have any crossed 2.0x since purchase? For exits: were these genuinely exhausted stories, or did you sell a potential tenbagger prematurely out of impatience? Flag any new diworsification positions — similar stories to existing holdings that dilute rather than diversify.`,

  graham: `Run the 9-point defensive checklist against each new equity addition. Does the margin of safety exceed 33% below estimated intrinsic value? Any addition failing more than 3 criteria is speculative and must be labeled as such — not hidden behind a growth narrative. For exits: were positions removed because they breached the safety criteria, or for unrelated reasons? Report the portfolio's overall defensive score: what percentage of current equity holdings would pass the full defensive investor standard?`,

  dalio: `Map each new or changed position to its All Weather quadrant sensitivity (Rising Growth / Falling Growth / Rising Inflation / Falling Inflation). Did the changes improve the portfolio's cross-quadrant balance, or deepen concentration in one economic regime? Given today's macro data, which quadrant is currently dominant — and is the portfolio over- or under-exposed to it? After these changes, do equities still dominate the portfolio's risk contribution disproportionately, or has risk parity improved?`,

  lilu: `Assign a conviction tier (High / Medium / Low / Unclear) to each new addition. Is each a business genuinely understood well enough to hold for 15 years — or a good business held with moderate conviction that merely seems acceptable? Was cash deployed into the highest-quality opportunities available, or simply deployed because holding cash felt uncomfortable? Are there still Low or Unclear positions occupying capital that belongs in the 5–8 highest-conviction ideas?`,

  ackman: `For each new position: state the variant perception (what the market misunderstands) and the named catalyst (specific event or management action) that will close the value gap in 3–5 years. Without clear answers to both, the position lacks activist edge. For exits: was the activist thesis fully realized, or was the position cut prematurely before the catalyst triggered? Assess the portfolio's macro downside protection: has it improved, stayed the same, or worsened since the last review?`,

  burry: `Did any change reduce the portfolio's single largest unhedged tail risk? Are new additions genuinely contrarian based on specific, verifiable data contradicting consensus — or consensus positions with a contrarian veneer? Name the exact macro shock (rate spike, credit freeze, dollar collapse, sovereign crisis) that would cause maximum damage today. Has any position been explicitly sized to hedge that tail risk, or is the portfolio still naked on it?`,

  fisher: `Apply Question 1 to each new addition: what do you specifically know about this business that the consensus has wrong or not yet priced in? Apply Question 2: what widely-held false belief makes a current holding a better opportunity than it appears? Did the changes improve or worsen alignment with the current market cycle phase? Has home-country concentration increased or decreased — and is the balance rational given where global sentiment is currently most extreme?`,

  cohen: `For each new position: name the specific near-term catalyst and the exact stop-loss price level set at entry. Are all existing stop-loss levels still active, current, and unbreached? For exits: were they triggered by defined stop-loss discipline, or delayed by rationalizing a recovery that never came? Calculate the portfolio heat score: what percentage of total capital is in active, catalyst-driven, time-sensitive positions — and has that score increased or decreased since the last review?`,

  marks: `Assess the pendulum position for each asset class now in the portfolio — is it swinging toward fear, at neutral, or toward dangerous complacency? For each new addition, apply second-level thinking explicitly: state the consensus view, then state what a superior thinker concludes instead. Given the current cycle phase, should this portfolio be calibrated for offense (maximizing return) or defense (protecting capital) — and do the changes reflect that judgment, or move against it?`,

  klarman: `For each new addition: what is the estimated intrinsic value and the margin of safety percentage at the purchase price? Was capital deployed into genuinely distressed, overlooked opportunities — or into widely-respected names that feel safe but carry no real discount to intrinsic value? For exits: were positions sold at or near full intrinsic value realization, or cut prematurely? Has the cash level remained appropriate to the current availability of genuine margin-of-safety opportunities?`,

  templeton: `Did new additions come from markets, sectors, or geographies currently at maximum pessimism — where fear has driven prices materially below long-term value? For exits: are positions being sold in areas that have already recovered from pessimism to fair value or beyond? Has global diversification improved toward the world's most unloved markets — or has concentration in comfortable, popular markets increased? Name one country or sector currently at or near maximum pessimism that the portfolio has zero exposure to.`,

  soros: `For each new position: identify the dominant market narrative it depends on and assess whether that narrative is self-reinforcing (still building toward a boom/bust inflection) or approaching its natural reversal point. Does every major position now have an explicit thesis-and-reversal plan? Has survival capacity improved: if the single largest position moved 25% adversely over 6 months, does the portfolio survive intact without forced selling?`,

  wood: `For each new addition: confirm its underlying technology or platform is on an active innovation S-curve — early adoption or crossing-the-chasm phase — not late-stage trend-following dressed as innovation. For exits: did the S-curve thesis genuinely fail, or was the position panic-sold during expected high-volatility drawdown? Has the portfolio's aggregate 5-year expected value increased since the last review — and does it remain significantly above today's total market value, justifying the innovation premium paid?`,

  druckenmiller: `For each new position: calculate the risk/reward ratio under the current macro regime — any position below 3:1 should not have been opened. Are the largest positions genuinely the highest-conviction, most macro-aligned ideas — or has comfortable legacy drift elevated low-conviction positions above their deserved weight? For exits: were they executed at clearly defined stop-loss levels, or held past the stop out of psychological attachment to entry prices?`,

  smith: `Run the quality filter on each new addition: does it sustain ROCE above 20%? Does FCF conversion exceed 80% of net profit? Does net debt stay below 2× EBITDA? Can the business be explained in one sentence? Any addition failing these criteria is not a Fundsmith-quality holding and should be acknowledged explicitly. For exits: were poor-quality or financially engineered businesses genuinely removed — or were good-quality businesses sold for short-term reasons the long-term thesis does not justify?`,

  greenblatt: `Score each new addition on the Magic Formula dual ranking: earnings yield (EBIT/EV) rank and ROIC rank. Do the additions land in the top composite quintile — or are they discretionary narrative additions that bypass the systematic criteria? Did the changes move the portfolio closer to a mechanically optimal basket, or did emotional reasoning crowd out statistically superior Formula-qualified positions? Are any current holdings past the 2–3 year systematic rebalancing window and due for replacement with higher-ranked candidates?`,
