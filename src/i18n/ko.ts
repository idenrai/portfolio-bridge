...
export const ko: Translations = {
  ...
  guru_philosophy_druckenmiller:
    "• 매크로 트레이딩의 달인: 통화, 금리, 주식 간 상관관계를 분석하여 방향성 베팅. 30년간 연평균 수익률 30% 달성\n" +
    "• 소로스의 수제자: 킀텀 펀드에서 검증된 통화·금리 전략을 계승·발전. '1992년 파운드 핀' 등 대형 매크로 베팅 성공\n" +
    "• 집중 베팅: 최고의 아이디어에는 자산의 상당 부분을 과감하게 베팅. 분산은 성과를 희석\n" +
    "• 모멘텀 추종: 추세가 있는 방향으로 오르되, 신호가 바뀌면 즉각 전환. 자존심보다 수익이 먼저\n" +
    "• 비대칭 리스크 관리: 손실을 제한하면서 수익을 극대화하는 비대칭 포지션 운용\n" +
    "• 명엸: '장기 수익률을 쌓는 방법은 두 가지다: 자본을 보전하고, 홈런을 쳐라.'",
  guru_philosophy_smith:
    "• 우량 기업 장기 보유: '좋은 기업을 사고, 과도하게 돈을 내지 말고, 아무것도 하지 마라(Buy good companies, don't overpay, do nothing)'\n" +
    "• 높은 ROIC 중시: 투하 자본 대비 꼸준히 높은 수익을 생성하는 기업만 투자. ROIC 20% 미만은 관심 무\n" +
    "• 집중 포트폴리오: 약 25~30개 종목 유지. '다각화는 무지에 대한 보호체일 뿐이다'\n" +
    "• 품질 우선: 저평가보다 사업 품질에 집중. 합리적 가격이면 매수, 과도하게 비싸지 않는 한 보유\n" +
    "• 복리 효과: 뛰어난 기업이 스스로 창출한 현금 흐름을 높은 수익률으로 재투자하는 복리 메커니즘\n" +
    "• 명엸: '펜드스미스 LLP는 주식이 다 함께 올라갈 때를 위해 최고의 기업만 실어 뀒코 있었다.'",
  guru_philosophy_greenblatt:
    "• 마법 공식(Magic Formula): 이익수익률(Earnings Yield)과 자본수익률(ROIC)이 높은 종목을 체계적으로 선별\n" +
    "• 정량적 가치 투자: 감정을 배제한 체계적·규칙 기반의 투자 접근. '마싵고 실용적으로 돈 범는 법'. 앱에도 체계화\n" +
    "• 소외된 주식 발굴: 시장이 무시한 저평가 우량 기업을 체계적으로 선별\n" +
    "• 분산 투자: 20~30개 종목에 분산하여 개별 종목 리스크 감소\n" +
    "• 인내가 필수: '마법 공식은 대부분 기간에 작동하지만, 가끔 3~4년 시장에 뒤지몐 수도 있다'\n" +
    "• 명엸: '맴리한 가격에 가치 있는 구실을 찾아라. 주식시장은 단기적으로는 미스터 마켓이다.'",
