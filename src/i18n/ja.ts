...
export const ja: Translations = {
  ...
  about_feat3_desc:
    "バフェット、ダリオ、リンチなど伝説の投賄家１15名の哲学と代表ポートフォリオを参考に、自分のポートフォリオと比較できます。",
  ...
  about_disclaimer:
    "このアプリは個人学習およびポートフォリオ管理を目的として制作されました。提供される相場情報・為替レート・分析は参考用であり、投賄判断の根拠として使用しないでください。",
  ...
  guru_philosophy_druckenmiller:
    "• マクロ取引の巨区：通貨、金利、株式の相関関係を分析した方向性ベット\n" +
    "• ソロスの直弟子：クォンタム・ファンドで証明済みの通貨・金利戦略を継承・発展\n" +
    "• 集中ベット：最善のアイデアに資産の相当部分を公然とベット\n" +
    "• モメンタム追従：勢いのある方向に乗り、シグナルが変れば即座に転換\n" +
    "• 非対称リスク管理：損失を限定しながら利益を最大化する非対称ポジション運用",
  guru_philosophy_smith:
    "• 優良企業の長期保有：「良い企業を買って何もするな」というシンプルな哲学\n" +
    "• 高いROIC重視：資本収益率が高く持続可能な企業にのみ投賄\n" +
    "• 集中ポートフォリオ：約25銘柄で構成された集中型ポートフォリオを維持\n" +
    "• 品質優先：割安より品質を重視、高くても良い企業なら購入\n" +
    "• 複利効果：優れた企業が創出するキャッシュフローが自己再投賄される複利メカニズム",
  guru_philosophy_greenblatt:
    "• 魔法の公式：高い益回り(Earnings Yield)と高い投賄資本利益率(ROIC)の組み合わせ\n" +
    "• 定量的バリュー投賄：感情を排した体系的・ルールベースの投賄アプローチ\n" +
    "• 見逃された銘柄の発掘：市場が無視した割安優良企業を体系的に選別\n" +
    "• 分散投賄：20～30銘柄に分散し個別銘柄リスクを低減\n" +
