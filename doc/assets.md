...
Displays all registered assets. Columns: Name, Market, Category, Qty, Avg Cost, Current Price, Value, P&L, Return.
...
Generates an English structured prompt listing all assets without a category, requesting JSON category assignments.
...
2. 5-row preview is displayed before confirmation
3. On confirm, rows are parsed and added to `useAssetStore`

Expected CSV columns: `name`, `ticker`, `type`, `market`, `currency`, `quantity`, `avgBuyPrice`, `currentPrice`, `category`
...
Exports the current asset list to a `.csv` file. All fields included. Implemented in `src/utils/csv.ts`.
