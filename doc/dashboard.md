...
Sortable table of all positions (including cash). Columns:
...
Table of currency exposure percentages with ±5% FX scenario analysis showing the monetary impact on total portfolio value.

---

### Rebalance Card (`RebalanceCard.tsx`)

Buy/sell amount suggestions to bring current allocation back to the configured targets. Shows:
...
Line chart of portfolio value over time. Data comes from `useSnapshotStore` — snapshots are saved periodically.

---

### AI Analysis Banner

Generates a structured portfolio analysis prompt via `buildInsightPrompt()` (`src/utils/ai/buildInsightPrompt.ts`).  
The user copies the prompt and pastes it into ChatGPT / Claude / Gemini / Grok.

See [ai-prompts.md](ai-prompts.md) for the full prompt specification.

---

### Onboarding

When the asset list is empty, the dashboard displays an onboarding state with a "Try with sample data" button.  
