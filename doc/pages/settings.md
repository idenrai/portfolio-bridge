...
The `plan3y`, `plan5y`, `plan10y`, and `notes` fields use a local `AutoResizeTextarea` component
defined in `Settings.tsx`. It automatically grows in height as content is typed,
using `useRef` + `useEffect` to set `height = scrollHeight`.
